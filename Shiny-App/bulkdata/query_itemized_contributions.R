...
query_itemized_contributions <- function(
   ...
   if (total_count==0) {return("no data to scrape")}
   else {
      ...
      tidy_donations$party<- as.character(lapply(tidy_donations$committee, `[[`, "party"))
      
      tidy_donations <- tidy_donations %>%
         select(party, report_year, fec_election_year, contribution_receipt_date, election_type, election_type_full, fec_election_type_desc, contributor_name, contributor_state, contributor_city, contributor_zip, contributor_occupation, is_individual, contribution_receipt_amount, committee_id, candidate_office_state)
